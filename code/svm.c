...
{
    clann_type *reference, multipliers[s->number_of_inputs];
    ...
    for (i = 0; i < s->number_of_inputs; i++)
        ...
        {
            total = sizeof(clann_type *) * (s->number_of_support_vectors + 1);
    ...
    if (s->kernel_type == SVM_TYPE_RBF || s->kernel_type == SVM_TYPE_MLP)
    {
         ...
         */
        s->weights = malloc(sizeof(clann_type) *
                            s->number_of_support_vectors);
...
svm_constraint_function(struct svm *s,
                        clann_type *l)
{
    clann_type c[s->number_of_inputs],
               c_sum = 0,
               error = 0;
    unsigned int i;
